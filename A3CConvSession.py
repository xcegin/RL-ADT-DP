...
trainer = tf.train.RMSPropOptimizer(learning_rate=0.001, decay=0.99, epsilon=0.01)
master_network = AConv_Network('global', None, num_feats)  # Generate global network
num_workers = multiprocessing.cpu_count()  # Set workers ot number of available CPU threads
workers = []
# Create worker classes
for i in range(num_workers):
    workers.append(ACConv_Worker(i, trainer, model_path, global_episodes, global_rewards))
