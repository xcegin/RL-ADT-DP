                        ...
                        for x in range(0, numOfTimes*5):
                            ...
                            while batch is not None:
                                ...
                                if exploration == "boltzmann":
                                    ...
                                    Q_d, allQ = sess.run([q_net.Q_dist, q_net.Q_out],
                                                         feed_dict={
                                                             ...
                                                             q_net.keep_per: 1.0})
                                    a = np.random.choice(Q_d[0][0], p=Q_d[0][0])
                                    a = np.argmax(Q_d[0][0] == a)
                                ...
                                if nextBatch is not None:
                                    ...
                                    batch = nextBatch
                                    myBuffer.add(
                                        np.reshape(np.array([nodes, children, nextNodes, nextChildren, a, r, d]),
                                                   [1, 7]))
                                if nextBatch is None:
                                    myBuffer.add(
                                        np.reshape(np.array([nodes, children, nodes, children, a, r, d]), [1, 7]))
                                running_reward += r
                                ...
                                if total_steps > pre_train_steps and total_steps % num_steps_upd == 0:
                                    ...
                                    Q1 = sess.run(q_net.predict,
                                                  feed_dict={q_net.nodes: np.vstack(trainBatch[:, 2]),
                                                             q_net.children: np.vstack(trainBatch[:, 3]),
                                                             q_net.keep_per: 1.0})
