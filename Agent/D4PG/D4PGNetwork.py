...
from Agent.D4PG.Model import build_actor, build_critic
from Agent.D4PG.network_utils import get_vars, copy_vars, l2_regularization
...
class QNetwork:
    ...
    def __init__(self, sess, gui, saver, buffer, feature_size):
        """
    ...
    def build_model(self):
        ...
        # Compute A(s_t)
        self.actions = build_actor(self.pooling, trainable=True, scope='learner_actor')

        # Compute Q(s_t, a_t)
        self.Q_distrib_given_actions = build_critic(self.pooling, self.action_ph,
                                                    trainable=True, reuse=False,
                                                    scope='learner_critic')

        # Compute Q(s_t, A(s_t)) with the same network
        self.Q_distrib_suggested_actions = build_critic(self.pooling, self.actions,
    ...
    def build_target(self):
        ...
        """
        # Compute A(s_{t+1})
        self.target_next_actions = build_actor(self.pooling,
                                               trainable=False,
                                               scope='learner_target_actor')

        # Compute Q_target( s_{t+1}, A(s_{t+1}) )
        self.Q_distrib_next = build_critic(self.pooling, self.target_next_actions,
