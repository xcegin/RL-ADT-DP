...
from Agent.ContinuousOwnEmbbed.ACContOwnWorker import ACOwnWorker
...
if __name__ == "__main__":
    ...
    with tf.device("/cpu:0"):
        global_ac = ACOwnNet(GLOBAL_NET_SCOPE, sess, num_feats)  # we only need its params
        workers = []
        # Create workers
        for i in range(num_workers):
            workers.append(ACOwnWorker(i, global_ac, sess, global_rewards, global_episodes))
